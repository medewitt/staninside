...
setup_stan_package <- function(loc = NULL, use_all = TRUE){
	...
	fs::file_copy(path = base_program,
	...
	if(use_all){
		...
		cli::cli_inform("To reference Stan code in expanded directories use `#include functions/func.stan syntax in the appropriate block`")
	}
